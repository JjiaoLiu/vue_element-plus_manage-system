...
export function initLineCharts(dom: HTMLDivElement, data: ChartData) {
  let legend = Object.keys(data.lineData);
  const option: ECOption = {
    backgroundColor: "#ffffff",
    title: {
      text: new Date().getMonth() + 1 + "月车流量统计图",
      top: 20,
      left: 20,
      textStyle: {
        color: "#333333",
    ...
    series: [
      ...
      },
      {
        ...
        data: data.lineData[legend[2]],
      },
    ],
  };
  echarts.init(dom).setOption(option);
}
export function initPieCharts(dom: HTMLDivElement, data: ChartData) {
  var img =
  ...
  var trafficWay = legend.map((l) => {
    let total = data.lineData[l].reduce((sum, pre) => {
      sum = sum + pre;
      return sum;
    }, 0);
    return {
